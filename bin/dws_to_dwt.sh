...
insert overwrite table ${APP}.dwt_uv_topic partition(dt='$do_date')
...
(
    select
        *
    from ${APP}.dwt_uv_topic where dt=date_add('$do_date',-1)
)old
full outer join
(
    select
...
(
    select *
    from ${APP}.dwt_user_topic where dt=date_add('$do_date',-1)
)old
full outer join
(
    select
...
(
    ...
    from ${APP}.dwt_sku_topic where dt=date_add('$do_date',-1)
)old
full outer join 
(
    select 
...
(
    select
        *
    from ${APP}.dwt_activity_topic where dt=date_add('$do_date',-1)
)old
full outer join
(
    select
...
(
    select
        *
    from ${APP}.dwt_area_topic where dt=date_add('$do_date',-1)
)old
full outer join
(
    select
